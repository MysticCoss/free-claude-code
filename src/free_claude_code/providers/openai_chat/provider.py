    ...
    def feed(self, chunk: Any) -> Iterator[str]:
        ...
        if self._output_reasoning:
            if self._structured_reasoning is not None:
                ...
                elif not delta.content and not self._output.has_content_block():
                    # Empty reasoning_content marks reasoning onset only
                    # before any block exists; relays like the OpenCode qwen
                    # gateway keep the field present with "" on every content
                    # chunk, and reopening blocks per chunk churns the stream.
                    yield from self._output.ensure_reasoning_block()

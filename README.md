...
**Claude Desktop 3P Mode.** Claude Desktop's third-party model mode only lists ids matching `claude-*` / `anthropic/claude-*`, hiding every FCC-advertised `anthropic/<provider>/<model>` id. Check "Claude Desktop 3P Mode" in the Admin UI (or set `ENABLE_CLAUDE_DESKTOP_3P=true`) to advertise each model as a single-segment `claude-<provider>-<model>` id (e.g. `deepseek/deepseek-v4-flash` → `claude-deepseek-deepseek-v4-flash`) and route those ids back to their provider on the way in.

...
**Claude Desktop 3P Mode.** Claude Desktop's third-party model mode only lists ids matching `claude-*` / `anthropic/claude-*`, hiding every FCC-advertised `anthropic/<provider>/<model>` id. Check "Claude Desktop 3P Mode" in the Admin UI (or set `ENABLE_CLAUDE_DESKTOP_3P=true`) to start a dedicated second listener (default port `8083`, configurable via `CLAUDE_DESKTOP_PORT`) that advertises each model as a single-segment `claude-<provider>-<model>` id (e.g. `deepseek/deepseek-v4-flash` → `claude-deepseek-deepseek-v4-flash`) and routes those ids back to their provider on the way in. The main port keeps serving the normal catalog untouched, so Claude Code and Claude Desktop can both be pointed at the same running proxy — just at different ports.
